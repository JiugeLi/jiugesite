...
import { 
  ...
  LayoutGrid, FolderOpen, Globe, MoreVertical, ChevronRight,
  Menu, X
} from 'lucide-react';
      ...
      <main className="flex-1 flex flex-col h-screen overflow-hidden relative bg-slate-50/50">
        ...
        <header className="px-6 lg:px-8 py-4 lg:py-6 flex items-center justify-between sticky top-0 z-20 glass mb-6">
          ...
          <div className="flex items-center gap-4 ml-auto lg:ml-0">
            {/* Mobile Search Icon - expands or shows modal? For now simple input */}
            <div className="lg:hidden">
              {/* Simplified mobile search if needed */}
            </div>

            {isAdmin && activeGroupId && !searchQuery && (
              <button
            ...
            {filteredWebsites.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-[50vh] text-center">
                ...
                <p className="text-slate-500 max-w-sm mx-auto leading-relaxed">
                  {searchQuery ? `We couldn't find anything matching "${searchQuery}". Try a different keyword.` : "This group is empty. Click the button above to add your first website!"}
                </p>
              </div>
            ) : (
                  ...
                  >
                    <div className="flex items-start gap-4">
                      <div className="w-14 h-14 flex-shrink-0 rounded-xl bg-slate-50 flex items-center justify-center overflow-hidden border border-slate-100 group-hover:scale-105 transition-transform duration-300">
                        {website.logo_url ? (
                          <img src={website.logo_url} alt={website.name} className="w-10 h-10 object-contain" />
                        ) : (
                    ...
                    {isAdmin && (
                      <div className="absolute top-3 right-3 opacity-0 group-hover:opacity-100 transition-opacity flex gap-1 bg-white/90 backdrop-blur-sm shadow-sm rounded-lg p-1 border border-slate-100">
                        <button
